---
inclusion: fileMatch
fileMatchPattern: "**/*{Quest,Task,Trigger,Tutorial,Dialogue,Fact,Objective}*.js"
---

# 任务中心制架构设计（Quest-Centric Architecture）

## 适用范围

本设计定义「任务中心制」的目标架构与分阶段实施路径：任务（Quest）成为策划的唯一编排单元，触发器、教程、对话降级为任务内可插步骤。结论来自现有 TaskGraph/Trigger/Tutorial/Dialogue 四系统分析、hero SLG 任务系统借鉴与用户裁定，实施按阶段①~④另行启动。
...
| P2 | **任务激活时不追认已成立事实**：任务图节点 active 前发生的事件永久丢失，读档后靠场景层 `reconcileTaskFacts` 补丁对账 | `example/sanguo_zhangjiao/scenes/BaseGameSceneSetup.js` L400-433 —— 场景层在做系统层的事（抽象倒置） |
...
| P4 | **教程完成事实多轨**：completedTutorials / signalProgress / onComplete 回调各说各话 | `src/systems/TutorialSystem.js` |
...
| P7 | **定义-存档兼容粒度**：已解决（per-trigger fingerprint），但任务图定义变更仍无对等机制 | `src/systems/TriggerSystem.js` L1148 |
...
| 系统 | 关键接口 | 位置 |
|------|---------|------|
| **TaskGraphSystem** | 定义 `{id,title,entryNodeId,reward,checkpoint,nodes[]}`；节点 objective 持 `eventMatcher{type,payload}` + `requiredCount` + `progressBy`；`consumeEvent` 幂等（eventIds）+ 影子事务提交；`getProjection` 供 HUD | `src/systems/TaskGraphSystem.js` L47-58, L277-320, L434-488 |
| **TriggerSystem** | 定义 `{id,when,if,do[],once,cooldown,reentryPolicy}`；`_runSteps` 串行 await + 每步 if 守卫 + branch + 教程 await 轮询；do 经 `actionDescriptorRegistry` + 命令层 | `src/systems/TriggerSystem.js` L896-1006, L743-826 |
| **TutorialSystem** | `showTutorial`（槽忙入 pendingTutorials FIFO）、`notify(signal)` 按 signalRules 计数完成 | `src/systems/TutorialSystem.js` L236, L259-263 |
| **DialogueSystem** | `startDialogue` 返回 bool；选项经 `setChoiceDispatcher` 派发器异步提交后 commit | `src/systems/DialogueSystem.js` L116-149, L227-295 |
| **QuestTransactionService** | `QUEST_COMMANDS`（quest.command/accept/advance/abandon/turnIn/track…）；`_executeTaskGraph` 影子提交 + 发布 `task.started/advanced/completed` | `src/systems/QuestTransactionService.js` L35-46, L209-303 |
| **CanonicalStateTransactionService** | 按 definitionId 解释 `commands[].transaction{when,writes}`，写黑板/StoryState，发布 `state.transaction.committed` | `src/systems/CanonicalStateTransactionService.js` L47-290 |
| **事件总线** | service 返回 committedEvents → PostCommitNotificationBus 有序分派 → SceneApplicationEventBridge 按 eventId 去重 + 重试 → `onContentEvent` 转发任务系统 | `src/core/scene/SceneApplicationEventBridge.js` L18-338；BaseGameSceneSetup.js L1010-1058 |

### 2.2 数据与注入点

- `example/sanguo_zhangjiao/game.project.json` 顶层键（行号）：quests **L2282（空数组，尚无内容）**、taskGraphs L2283（2 条）、dialogues L2133（10 条）、triggerCatalog L2586、triggers L3475（113 条）、tutorials L6053（17 条）、commands L7922（40 条 state.transaction）。
- **taskGraphs 注入点已存在**：GameSceneRuntime `config.taskDefinitions` 构造 TaskGraphSystem（`src/core/scene/GameSceneRuntime.js` L176-185）。
- **triggers 注入点已存在**：GameLoader → `triggerSystem.registerAll`（`src/core/GameLoader.js` L491）。
...
- 编辑器：TaskGraphEditor.js（scene-workflow.html 的 task 标签，identityFields 下拉 L212-295）、TriggerStorylinePanel.js（动作编排 L326-380）、QuestEditor.js 已存在（实施前需确认现状）；保存统一 `fetch POST /api/save-file`。
...
事件契约登记落地于 TriggerCatalog 引擎级 CORE_EVENTS（阶段①已实施）：`gathering.completed` / `state.transaction` / `enemy.killed` 已登记 payload 契约（paramsSchema）、身份字段与来源系统标注（source），任务编辑器无需依赖项目级 triggerCatalog 登记即可见完整契约——以单一事件目录避免双目录漂移。
...
- 实施前先读 `editor/QuestEditor.js` 确认现状（该文件已存在，可能部分复用）。
...
各阶段验证通用基线：现有测试（s01FirstWolfPack.test.js、taskGraphWoodProgress.test.js、TriggerSystem.test.js、PlacementSpawner.count.test.js）保持全绿。

## 5. 设计裁定

- **运行时编译**而非编译期展开（3.1 已述理由）。
- **场景归属**（用户裁定）：Quest 增加 `scenes` 字段，编译触发器继承 editorScope.sceneIds；留空 = 全局跨场景任务；任务实例进度全局持久（见 3.3 场景归属语义）。
