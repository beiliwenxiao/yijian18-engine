...
- 跨 Region 的九宫格或其他全局坐标消费者必须先构建 `ProjectWorldIndex`，再通过 `getGlobalCells(row,col)` 读取全部 Region 的只读 coverage；重复的八邻域需求统一调用 `getAdjacentSceneAnchors(sceneId)`，禁止各宿主复制坐标循环。默认只返回 loadable 场景，禁止在调用方重新解释 raw grid、遗漏其他 Region 或把 reserved 当作可加载场景。相对预览位置使用两个 anchor 的 `offset` 差值，不能假定当前 Region 的 chunk 尺寸适用于所有邻居。
